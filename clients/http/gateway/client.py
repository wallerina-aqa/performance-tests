...
def build_gateway_locust_http_client(environment: Environment) -> Client:
    """
    HTTP-клиент, предназначенный специально для нагрузочного тестирования
                                                                 с помощью Locust.
    ...
    (время ответа, длину ответа и т.д.) и отправляет их в Locust
                                           через `environment.events.request`.
