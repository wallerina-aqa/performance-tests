...
make_top_up_operation_view_url = f"{base_url}/operations/make-top-up-operation"
card_id = open_debit_card_account_response_data["account"]["cards"][0]["id"]
account_id = open_debit_card_account_response_data["account"]["id"]
make_top_up_operation_view_payload = {
    "status": "COMPLETED",
    "amount": 1500,
    "cardId": card_id,
...
make_top_up_operation_view_response = httpx.post(
    url=make_top_up_operation_view_url, json=make_top_up_operation_view_payload
)
make_top_up_operation_view_status_code = make_top_up_operation_view_response.status_code
make_top_up_operation_view_response_data = make_top_up_operation_view_response.json()
print(f"Status code: {make_top_up_operation_view_status_code}")
print(f"Make top up operation response: {make_top_up_operation_view_response_data}")
